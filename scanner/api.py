"""
SafeNet Ghana - API Server v0.4.0
Vulnerability Scanner + WiFi IDS + PostgreSQL + JWT Auth
...
from models   import Base, ScanResult, WifiAlert, SystemLog
...
# ─────────────────────────────────────────────
#  WIFI ALERT DB CALLBACK
#  Every time the IDS detects an attack,
#  this function saves it to PostgreSQL.
# ─────────────────────────────────────────────
def save_wifi_alert_to_db(alert: dict):
    """Called automatically by the WiFi IDS on each detection."""
    try:
...
# Attach the DB callback to the IDS
...
def trigger_scan(request: ScanRequest, current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    ...
    print(f"\n[SCAN] {current_user.username} → {request.target} ({request.scan_type})")
    ...
    db_scan = ScanResult(
        ...
        scanned_by=current_user.username, overall_risk=summary["overall_risk"],
        total_hosts=summary["total_hosts"], open_ports=summary["open_ports"],
        critical_count=summary["critical_count"], high_count=summary["high_count"],
        medium_count=summary["medium_count"], low_count=summary["low_count"],
        full_report=report,
    )
    db.add(db_scan)
    db.commit()
    ...
    print(f"[SCAN] Done → risk={summary['overall_risk']} id={db_scan.id}")
...
def start_wifi_ids(request: WifiStartRequest, current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    """Start the WiFi IDS. Admin only."""
    if wifi_ids.running:
        return {"status": "already_running", "detail": "WiFi IDS is already active."}
    ...
    log_to_db(db, "INFO", "WIFI", f"WiFi IDS started by {current_user.username} on interface: {request.interface or 'default'}")
    return result

@app.post("/wifi/stop")
def stop_wifi_ids(current_user: User = Depends(require_admin), db: Session = Depends(get_db)):
    """Stop the WiFi IDS. Admin only."""
...
def wifi_status(current_user: User = Depends(get_current_user)):
    """Current IDS status — running, packets seen, alert count."""
...
def get_wifi_alerts(
    ...
    """
    Returns WiFi alerts.
    Combines live in-memory alerts with historical DB alerts.
    """
    # Live alerts from memory
